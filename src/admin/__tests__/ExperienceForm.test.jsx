...
describe('ExperienceForm Component', () => {
  ...
  it('renders standard form elements correctly', () => {
    renderForm();
    expect(screen.getByText('Pengalaman Baru')).toBeInTheDocument();
    ...
    expect(screen.getByLabelText(/Tanggal Berakhir/)).toBeInTheDocument();
    expect(screen.getByText(/Simpan Pengalaman/i)).toBeInTheDocument();
  });

  it('validates required fields on submit (Role, Company, Start Date)', async () => {
    renderForm();
    const saveButton = screen.getByText(/Simpan Pengalaman/i);
    ...
    fireEvent.change(screen.getByLabelText(/Nama Perusahaan/), { target: { value: 'Google' } });
    fireEvent.change(screen.getByLabelText(/Tanggal Mulai/), { target: { value: '2023-01-01' } });
  ...
  it('adds and removes dynamic responsibilities correctly', async () => {
    ...
    const addButton = screen.getByText(/Tambah Item/i);
    
    // Initial state: 0 items
    expect(screen.queryByPlaceholderText('Tanggung Jawab #1')).not.toBeInTheDocument();

    // Add first item
    fireEvent.click(addButton);
    expect(screen.getByPlaceholderText('Tanggung Jawab #1')).toBeInTheDocument();

    // Add second item
    fireEvent.click(addButton);
    expect(screen.getByPlaceholderText('Tanggung Jawab #2')).toBeInTheDocument();

    // Type into first item
    const input1 = screen.getByPlaceholderText('Tanggung Jawab #1');
    ...
    expect(screen.getByPlaceholderText('Tanggung Jawab #1')).toBeInTheDocument();
    expect(screen.queryByPlaceholderText('Tanggung Jawab #2')).not.toBeInTheDocument();
    expect(screen.getByPlaceholderText('Tanggung Jawab #1').value).toBe('');
  });

  it('handles "Currently working here" checkbox carefully', async () => {
    renderForm();
    const endDateInput = screen.getByLabelText(/Tanggal Berakhir/);
    const checkbox = screen.getByLabelText(/Masih bekerja di sini/i);
  ...
  it('submits correctly formatted payload handling empty responsibilities to supabase', async () => {
    ...
    // Fill valid data
    fireEvent.change(screen.getByLabelText(/Peran/), { target: { value: 'Frontend Dev' } });
    fireEvent.change(screen.getByLabelText(/Nama Perusahaan/), { target: { value: 'Vercel' } });
    fireEvent.change(screen.getByLabelText(/Tanggal Mulai/), { target: { value: '2022-05-10' } });

    // Check "Currently working here"
    fireEvent.click(screen.getByLabelText(/Masih bekerja di sini/i));

    // Add responsibilities
    fireEvent.click(screen.getByText(/Tambah Item/i));
    fireEvent.click(screen.getByText(/Tambah Item/i));
    fireEvent.click(screen.getByText(/Tambah Item/i));

    // Fill 1 valid, 1 with spaces, 1 empty
    const inputs = screen.getAllByRole('textbox').filter(i => i.placeholder?.startsWith('Tanggung Jawab'));
